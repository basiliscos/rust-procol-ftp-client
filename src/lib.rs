...
impl fmt::Display for State {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            &State::DataTypeReqSent(ref value) => {
                write!(f, "[state: data-type-req-sent({})]", value)
            }
            &State::DataTypeConfirmed(ref value) => {
                write!(f, "[state: data-type-confirmed({})]", value)
            }
            &State::SystemRecived(ref name, ref subtype) => {
                write!(f, "[state: system-recieved({}/{})]", name, subtype)
            }
            &State::PassiveConfirmed(ref addr, ref port) => {
                write!(f, "[state: passive-mode ({}:{})]", addr, port)
            }
            _ => {
...
impl FtpReceiver {
    ...
    fn advance_state(prev_state: &State,
                     prev_req: &Option<Rc<State>>,
                     bytes: &[u8])
                     -> Result<State, FtpError> {
      ...
      static ref RE_PASSIVE_MODE: Regex =
        Regex::new("Entering Passive Mode \\((\\d+),(\\d+),(\\d+),(\\d+),(\\d+),(\\d+)\\)")
        .unwrap();
    }
        ...
        str::from_utf8(bytes)
                            ...
                            OPERATION_SUCCESS => {
                                match &*prev_req {
                                    &Some(ref prev_sent_req) => {
                                        match &**prev_sent_req {
                                            &State::DataTypeReqSent(ref value) => {
                                                Ok(State::DataTypeConfirmed(value.clone()))
                                            }
                            ...
                            SYSTEM_RECEIVED => {
                                ...
                                RE_SYSTEM.captures(system_str)
                                    ...
                                    .and_then(|path_capture| {
                                        ...
                                        Ok(State::SystemRecived(name.to_string(),
                                                                subtype.to_string()))
                                    })
                            }
                            PASSIVE_MODE => {
                                ...
                                RE_PASSIVE_MODE.captures(addr_str)
                                    ...
                                    .and_then(|path_capture| {
                                        let mut numbers =
                                            path_capture.iter().skip(1).map(|opt_value| {
            ...
            .and_then(|new_state| {
                ...
                if allowed {
                    ...
                    Err(FtpError::ProtocolError(format!("{} => {} is not allowed",
                                                        prev_state,
                                                        new_state)))
                }
            })
    ...
    pub fn try_advance(self, buffer: &[u8]) -> Result<FtpTransmitter, Self> {
        ...
        let transition_result =
            FtpReceiver::advance_state(&internals.state, &internals.sent_request, buffer);
            ...
            Ok(new_state) => {
                {
                    ...
                    let final_state = match new_state {
                        ...
                        State::DataTransferCompleted => State::Authorized,
                        _ => new_state,
                    };
...
impl FtpTransmitter {
        ...
        match &*internals.state {
            ...
            &State::LoginReady => {
                ...
                unsafe {
                    ...
                    ptr::copy_nonoverlapping(&data_login[0],
                                             &mut buffer[my_count],
                                             data_login.len());
                    my_count += data_login.len();
                    ptr::copy_nonoverlapping(&DATA_ENDING[0],
                                             &mut buffer[my_count],
                                             DATA_ENDING.len());
                };
        ...
        match &*internals.state {
            &State::PasswordExpected => {
                ...
                unsafe {
                    ...
                    ptr::copy_nonoverlapping(&data_password[0],
                                             &mut buffer[my_count],
                                             data_password.len());
    ...
    pub fn send_pwd_req(self, buffer: &mut [u8], count: &mut usize) -> FtpReceiver {
        ...
        match &*internals.state {
            &State::Authorized => {
                unsafe {
                    ptr::copy_nonoverlapping(&DATA_PWD[0], &mut buffer[0], DATA_PWD.len());
                }
                *count = DATA_PWD.len();
                {
                    let mut int_ref = Rc::get_mut(&mut internals).unwrap();
        ...
        match &*internals.state {
            &State::Authorized => {
                match &data_type {
                    &DataMode::Binary => {
                        unsafe {
                            ptr::copy_nonoverlapping(&DATA_DATA_BINARY[0],
                                                     &mut buffer[0],
                                                     DATA_DATA_BINARY.len());
                        }
                        *count = DATA_DATA_BINARY.len();
                    }
                    &DataMode::Text => {
                        unsafe {
                            ptr::copy_nonoverlapping(&DATA_DATA_TEXT[0],
                                                     &mut buffer[0],
                                                     DATA_DATA_TEXT.len());
                        }
    ...
    pub fn send_system_req(self, buffer: &mut [u8], count: &mut usize) -> FtpReceiver {
        ...
        match &*internals.state {
            &State::Authorized => {
                unsafe {
                    ptr::copy_nonoverlapping(&DATA_SYST[0], &mut buffer[0], DATA_SYST.len());
                }
                *count = DATA_SYST.len();
                {
                    let mut int_ref = Rc::get_mut(&mut internals).unwrap();
    ...
    pub fn send_pasv_req(self, buffer: &mut [u8], count: &mut usize) -> FtpReceiver {
        ...
        match &*internals.state {
            &State::Authorized => {
                unsafe {
                    ptr::copy_nonoverlapping(&DATA_PASV[0], &mut buffer[0], DATA_PASV.len());
                }
                *count = DATA_PASV.len();
                {
                    let mut int_ref = Rc::get_mut(&mut internals).unwrap();
    ...
    pub fn send_list_req(self, buffer: &mut [u8], count: &mut usize) -> FtpReceiver {
        ...
        match &*internals.state {
            &State::Authorized => {
                unsafe {
                    ptr::copy_nonoverlapping(&DATA_LIST[0], &mut buffer[0], DATA_LIST.len());
                }
                *count = DATA_LIST.len();
                {
                    let mut int_ref = Rc::get_mut(&mut internals).unwrap();
      ...
      static ref RE_FILE: Regex =
        ...
        str::from_utf8(data)
            ...
            .and_then(|list| {
                ...
                let files = line_captures.filter_map(|line_cap| {
